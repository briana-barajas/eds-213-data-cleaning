...
snowsurvey_clean <- snowsurvey_clean %>% 
  ...
  # remove columns where all values are NA                                      #<--JB probably meant rows
  filter(!is.na(snow_cover) | !is.na(water_cover) | !is.na(land_cover)) %>%     #<--JB seems convoluted.. what about filter(!(is.na(snow_cover) & is.na(water_cover) & is.na(land_cover)))
  
  # convert column to numeric
  mutate(total_cover = as.numeric(total_cover)) %>% 
  
  # remove column above 100
  filter(total_cover <= 100)                                                    #<--JB that would be better as a check to better understand why this could happen instead,  of dropping it; some values are slightly above 100; -1pt
...
#<--JB: What about the case when there is only one value, but it is 100%? -1pt
